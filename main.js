...
function Beautify(val,floats,format)
{
	...
	var output=(val.toString().indexOf('e+')!=-1 && format==2)?val.toPrecision(4).toString():formatter(val).toString().replace(/\B(?=(\d{3})+(?!\d))/g,',');
		...
		{name:'store',nameD:'Store',icon:2,init:function(){
			...
			G.storeSections=[
				{name:'buildings',nameD:'Buildings',icon:5,content:function(el){
					...
					el.innerHTML=`<div class="scrollableList storeList" style="overflow-y:scroll;">${str}</div>`;
		...
		{name:'misc',nameD:'Misc',icon:4,build:function(el){
			...
			str+=G.button({text:`${G.iconSmall(17,5)}About`,classes:'bumpButton halfButton',onclick:function(){
				G.popup({text:`
					...
					linkTo('Wiki<div style="position:absolute;left:0px;right:0px;bottom:6px;padding:4px;font-size:8px;opacity:0.75;">the unofficial wiki</div>','https://cookieclicker.wiki.gg/wiki/Cookie_Clicker_Wiki')+
...
//===========================================================================
	...
	G.popup=function(o)
	{
		...
		let el=document.createElement('div');
		el.className='popup '+(o.introAnim||(G.getSet('lowMotion')?'':'plop'))+(o.classes?(' '+o.classes):'');
